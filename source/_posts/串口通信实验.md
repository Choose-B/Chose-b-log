...
## 串口收发通信中断
处在 串口接受通信中断函数`HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)`中的代码即为触发串口收发通信中断时执行的代码  
当一个串口的`tx`端向另外一个串口的`rx`端发送一串`uint8_t`的数据,接收到数据的串口会调用一次*串口接受通信中断函数*  
同样的,在项目的代码库中这个函数以*弱函数*的方式被定义,因此我们在**USER CODE 0**中可以重新定义一个*串口接受通信中断函数*  
也不要忘了要在**USER CODE 2**中打开*中断接受*`HAL_UART_Receive_IT(&huart_n,rxbuffer,sizeof(rxbuffer));`哦   
**其中rxbuffer是用来接受数据的数组,变量类型必须是uint8_t**  

除了要写接受端的代码,我们当然还要写发送端的代码  
...
```
*串口接收中断回调函数*中的指针`huart`指向*接受到数据并触发此次中断的串口*  
利用这个指针可以判断到底是哪个串口接受到了数据  
```c
//数据发送数组
uint8_t txbuffer[2]={0,1};
//数据接受数组
